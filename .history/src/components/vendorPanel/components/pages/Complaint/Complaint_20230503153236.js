  ...
  const Complaints = async () => {
    try {
      ...
      const { data } = await axios.get(
        "https://ou6tgbmd2b.execute-api.ap-south-1.amazonaws.com/dev/api/v1/complaints"
      );
      setData(data);
      console.log(data);
      setLoading(false);
  ...
  const deleteComplaint = async (id) => {
    try {
      const data = await axios.delete(
        `https://ou6tgbmd2b.execute-api.ap-south-1.amazonaws.com/dev/api/v1/complaints/${id}`
      );
